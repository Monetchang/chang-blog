...
| 模块名称      | 功能描述                                         | 核心函数 / 调用点                                                              | 设计亮点                                    |
| --------- | -------------------------------------------- | ----------------------------------------------------------------------- | --------------------------------------- |
| 用户上传文档    | 接收用户上传的 PDF / DOCX / Markdown / 图片文件，校验后传入后端 | `/upload_and_parse`                                                     | 支持多文件并行上传与类型自动识别                        |
| 定位知识库     | 通过会话 ID 获取关联知识库（Knowledgebase）               | `ConversationService.get_by_id()`<br>`KnowledgebaseService.get_by_id()` | 动态知识库绑定机制，支持多租户隔离                       |
| 存储与初步分析   | 将文件保存至对象存储，提取基础元数据并登记数据库                     | `FileService.upload_document()`                                         | 异步上传，提升系统并发性能                           |
| 文档解析与智能切块 | 按文件类型调用解析器（PPT、图片、音频、邮件或默认）进行语义切块            | `FACTORY.get(...).chunk()`                                              | ✅ 非图片类文档自动生成 **思维导图（MindMap）** 并存储为关联内容 |
| 生成索引      | 为每个内容块生成唯一 ID 与索引结构                          | 内部索引构建逻辑                                                                | 支持多模态索引体系（文本 / 图片 / 思维导图）               |
| 向量化入库     | 使用 Embedding 模型生成语义向量并写入向量数据库                | `LLMBundle.encode()`<br>`docStoreConn.insert()`                         | 多模型兼容，批量加速推理，提高嵌入生成效率                   |
| 返回结果      | 更新文档统计信息并返回上传结果（文档 ID 列表）                    | `DocumentService.increment_chunk_num()`                                 | 实时文档入库状态反馈机制                            |
