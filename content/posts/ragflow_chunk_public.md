...
| 模块名称      | 核心函数 / 调用点                                                              | 功能描述                                                   | 设计亮点                                     |
| --------- | ----------------------------------------------------------------------- | ------------------------------------------------------ | ---------------------------------------- |
| 用户上传文档    | `upload_and_parse()`                                                    | 接收用户上传的 PDF / DOCX / Markdown / 图片文件，校验文件合法性后传入后端处理    | 支持多文件并行上传与类型自动识别                         |
| 定位知识库     | `ConversationService.get_by_id()`<br>`KnowledgebaseService.get_by_id()` | 通过会话 ID 获取关联知识库（Knowledgebase），确保文档与知识库关联              | 动态知识库绑定机制，支持多租户隔离                        |
| 存储与初步分析   | `FileService.upload_document()`                                         | 将上传文件保存至对象存储，提取基础元数据（类型、大小、页数等）并登记数据库                  | 异步上传，提升系统并发性能                            |
| 文档解析与智能切块 | `FACTORY.get(...).chunk()`                                              | 根据文件类型（PPT、图片、音频、邮件、默认）调用对应解析器进行语义切块                   | ✅ 对非图片类文档自动生成 **思维导图（MindMap）** 并存储为关联内容 |
| 生成索引      | 内部生成唯一 chunk ID 与索引结构体                                                  | 为每个内容块生成唯一 ID 与内容索引结构                                  | 支持多模态索引体系（文本 / 图片 / 思维导图）                |
| 向量化入库     | `LLMBundle.encode()`<br>`settings.docStoreConn.insert()`                | 对 chunk 内容调用 Embedding 模型生成向量并写入向量数据库（如 Elasticsearch） | 多模型兼容，批量加速推理，提高嵌入生成效率                    |
| 返回结果      | `DocumentService.increment_chunk_num()`                                 | 更新文档统计信息并返回上传结果（文档 ID 列表）                              | 实时文档入库状态反馈机制                             |
